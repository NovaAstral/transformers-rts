...
function ENT:Initialize()
	util.PrecacheModel("models/starcraft/terran/building/bunker.mdl")
	
	self.Entity:SetModel("models/starcraft/terran/building/bunker.mdl")
...
function ENT:OnTakeDamage(damage)
	...
	if (!self.Exploded) then
		...
		if(self.Entity:Health() <= self.CriticalHP and !self.smoke and !IsValid(self.smoke)) then
			...
			hook.Add("Think","Bunker_Smoke_RemoveOnHeal",function()
				if(self.Entity:Health() > self.CriticalHP) then
					self:StopSmoke()
					hook.Remove("Think","Bunker_Smoke_RemoveOnHeal")
				end
			end)
		end
	end
...
function ENT:Splode() --Explode the entity
	if(!self.Exploded) then
		...
		timer.Create("Bunker_Explode1"..self:EntIndex(),1,3,function() --3 Small Explosions
		...
		timer.Create("Bunker_Explode2"..self:EntIndex(),5,1,function() --main explosion
			...
			NScreenShake("bunker_shake",200,750,5,255,self.Entity:GetPos())
...
function ENT:OnRemove()
	hook.Remove("Think","Bunker_Smoke_RemoveOnHeal")
	timer.Stop("Bunker_Explode1"..self:EntIndex())
	timer.Stop("Bunker_Explode2"..self:EntIndex())
end