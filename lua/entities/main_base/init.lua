...
function ENT:SpawnFunction(ply, tr)
	local ent = ents.Create("main_base")
...
function ENT:OnTakeDamage(damage)
	...
	if (!self.Exploded) then
		...
		if(self.Entity:Health() <= self.CriticalHP and !self.smoke and !IsValid(self.smoke)) then
			...
			hook.Add("Think","MainBase_Smoke_RemoveOnHeal",function()
				if(self.Entity:Health() > self.CriticalHP) then
					self:StopSmoke()

					--stop alarm sound

					hook.Remove("Think","MainBase_Smoke_RemoveOnHeal")
				end
			end)
		end
	end
...
function ENT:Splode() --Explode the entity
	if(!self.Exploded) then
		...
		timer.Create("MainBase_Explode1",1,3,function() --3 Small Explosions
		...
		timer.Create("MainBase_Explode2",5,1,function() --main explosion
			...
			NScreenShake("mbase_shake",200,750,5,255,self.Entity:GetPos())
...
function ENT:OnRemove()
	hook.Remove("Think","MainBase_Smoke_RemoveOnHeal")
	timer.Stop("MainBase_Explode1")
	timer.Stop("MainBase_Explode2")
end