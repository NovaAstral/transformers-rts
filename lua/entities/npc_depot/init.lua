...
function ENT:SpawnFunction(ply, tr)
	local ent = ents.Create("npc_depot")
...
function ENT:Initialize()
	util.PrecacheModel("models/starcraft/terran/building/factory.mdl")
	
	self.Entity:SetModel("models/starcraft/terran/building/factory.mdl")
...
function ENT:Use(ply)
	if(IsValid(ply) and !self.Exploded) then
		...
		spawnent:SetPos(self.Entity:GetPos()+Vector(math.random(-100,100),math.random(-100,100),100))
		spawnent:Activate()
		spawnent:Spawn()
	end
end
...
function ENT:OnTakeDamage(damage)
	...
	if (!self.Exploded) then
		...
		if(self.Entity:Health() <= self.CriticalHP and !self.smoke and !IsValid(self.smoke)) then
			...
			hook.Add("Think","ResDep_Smoke_RemoveOnHeal",function()
				if(self.Entity:Health() > self.CriticalHP) then
					self:StopSmoke()
					hook.Remove("Think","ResDep_Smoke_RemoveOnHeal")
				end
			end)
		end
	end
...
function ENT:Splode() --Explode the entity
	if(!self.Exploded) then
		...
		timer.Create("NPCDep_Explode1",1,3,function() --3 Small Explosions
		...
		timer.Create("NPCDep_Explode2",5,1,function() --main explosion
			...
			NScreenShake("npcdepot_shake",200,750,5,255,self.Entity:GetPos())
...
function ENT:OnRemove()
	hook.Remove("Think","ResDep_Smoke_RemoveOnHeal")
	timer.Stop("NPCDep_Explode1")
	timer.Stop("NPCDep_Explode2")
end