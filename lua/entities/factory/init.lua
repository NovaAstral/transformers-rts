...
function ENT:Use(ply)
	if(IsValid(ply) and !self.Exploded) then
		...
		spawnent:SetPos(self.Entity:LocalToWorld(Vector(0,0,100)))
...
function ENT:OnTakeDamage(damage)
	...
	if (!self.Exploded) then
		...
		if(self.Entity:Health() <= self.CriticalHP and !self.smoke and !IsValid(self.smoke)) then
			...
			hook.Add("Think","Factory_Smoke_RemoveOnHeal",function()
				if(self.Entity:Health() > self.CriticalHP) then
					self:StopSmoke()
					hook.Remove("Think","Factory_Smoke_RemoveOnHeal")
				end
			end)
		end
	end
...
function ENT:Splode() --Explode the entity
	if(!self.Exploded) then
		...
		timer.Create("Factory_Explode1"..self:EntIndex(),1,3,function() --3 Small Explosions
		...
		timer.Create("Factory_Explode2"..self:EntIndex(),5,1,function() --main explosion
			...
			NScreenShake("factory_shake",200,750,5,255,self.Entity:GetPos())
...
function ENT:OnRemove()
	hook.Remove("Think","Factory_Smoke_RemoveOnHeal")
	timer.Stop("Factory_Explode1"..self:EntIndex())
	timer.Stop("Factory_Explode2"..self:EntIndex())
end