...
function ENT:SpawnFunction(ply, tr)
	local ent = ents.Create("barracks")
...
function ENT:Initialize()
	util.PrecacheModel("models/starcraft/terran/building/barracks.mdl")
	
	self.Entity:SetModel("models/starcraft/terran/building/barracks.mdl")
...
function ENT:Use(ply)
	if(IsValid(ply) and !self.Exploded) then
		local spawnent = ents.Create("npc_citizen")
		spawnent:SetPos(self.Entity:LocalToWorld(Vector(350,0,0))+Vector(math.random(-100,100),math.random(-100,100),10))
		spawnent:Activate()
		spawnent:Spawn()
	end
end
...
function ENT:OnTakeDamage(damage)
	...
	if (!self.Exploded) then
		...
		if(self.Entity:Health() <= self.CriticalHP and !self.smoke and !IsValid(self.smoke)) then
			...
			hook.Add("Think","Barracks_Smoke_RemoveOnHeal",function()
				if(self.Entity:Health() > self.CriticalHP) then
					self:StopSmoke()
					hook.Remove("Think","Barracks_Smoke_RemoveOnHeal")
				end
			end)
		end
	end
...
function ENT:Splode() --Explode the entity
	if(!self.Exploded) then
		...
		timer.Create("Barracks_Explode1"..self:EntIndex(),1,3,function() --3 Small Explosions
		...
		timer.Create("Barracks_Explode2"..self:EntIndex(),5,1,function() --main explosion
			...
			NScreenShake("barracks_shake",200,750,5,255,self.Entity:GetPos())
...
function ENT:OnRemove()
	hook.Remove("Think","Barracks_Smoke_RemoveOnHeal")
	timer.Stop("Barracks_Explode1"..self:EntIndex())
	timer.Stop("Barracks_Explode2"..self:EntIndex())
end